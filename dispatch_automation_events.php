...
/**
 * Hostinger cron target. Run once per minute with PHP CLI.
 * This file intentionally refuses browser requests.
 */
if (PHP_SAPI !== 'cli') {
...
while ($event = $events->fetch_assoc()) {
    ...
    $signaturePayload = $timestamp . '.' . $event['payload_json'];
    $signature = hash_hmac('sha256', $signaturePayload, automationConfig('TTR_AUTOMATION_WEBHOOK_SECRET'));
    $headers = [
